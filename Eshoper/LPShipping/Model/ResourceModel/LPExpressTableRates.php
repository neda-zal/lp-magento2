...
class LPExpressTableRates extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb
{
    /**
    ...
    /**
     * @var \Eshoper\LPShipping\Model\LPExpressTableRatesFactory $_LPExpressTableRatesFactory
     */
     ...
     * @param \Eshoper\LPShipping\Model\LPExpressTableRatesFactory $LPExpressTableRatesFactory
     * @param null $connectionName
     */
    public function __construct(
        \Magento\Framework\Model\ResourceModel\Db\Context $context,
        \Magento\Framework\Filesystem $filesystem,
        \Eshoper\LPShipping\Model\LPExpressTableRatesFactory $LPExpressTableRatesFactory,
        $connectionName = null
    ) {
    ...
    public function uploadAndImport ( \Magento\Framework\DataObject $object )
    {
        ...
        if ( ! empty ( $data ) && $this->validate ( $data ) ) {
            ...
            foreach ( $data as $rate ) {
                /** @var \Eshoper\LPShipping\Model\LPExpressTableRates $rates */
                $this->_LPExpressTableRatesFactory->create ()
                    ->setWeightTo ( $rate [ 'weight_to' ] )
                    ->setCourierPrice ( $rate [ 'lpexpress_courier_price' ] )
                    ->setTerminalPrice ( $rate [ 'lpexpress_terminal_price' ] )
                    ->setPostOfficePrice ( $rate [ 'lpexpress_postoffice_price' ] )
                    ->save ();
            }
        }
    }
    ...
    {
        $weights = $result = [];
        $rates = $this->_LPExpressTableRatesFactory->create ()->getCollection ()->getData ();
    ...
    private function getData ( \Magento\Framework\Filesystem\File\ReadInterface $file )
    {
        ...
        while ( false !== ( $csvLine = $file->readCsv() ) ) {
             ...
             * Columns
             * 0 - weight_to
             * 1 - lpexpress_courier_price
             * 2 - lpexpress_terminal_price
             * 3 - lpexpress_postoffice_price
             */
            array_push (
                $data,
                [
                    'weight_to' => $csvLine [ 0 ],
                    'lpexpress_courier_price' => $csvLine [ 1 ],
                    'lpexpress_terminal_price' => $csvLine [ 2 ],
                    'lpexpress_postoffice_price' => $csvLine [ 3 ],
                ]
            );
        }
